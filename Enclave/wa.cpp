...
Type block_types[5] = {
    { .form = BLOCK, .param_count=0, .params=0, .result_count = 0, .results=0},
    { .form = BLOCK, .param_count=0, .params=0,.result_count = 1, .results = block_type_results[0], },
    { .form = BLOCK, .param_count=0, .params=0,.result_count = 1, .results = block_type_results[1], },
    { .form = BLOCK, .param_count=0, .params=0,.result_count = 1, .results = block_type_results[2], },
    { .form = BLOCK, .param_count=0, .params=0,.result_count = 1, .results = block_type_results[3], }
};
...
Block *pop_block(Module *m) {
    ...
    if (t->result_count == 1) {
        if (m->stack[m->sp].value_type != t->results[0]) {
            snprintf(exception, 1024, "call signature mismatch");
            return NULL;
        }
    }
...
bool interpret(Module *m) {
    ...
    while (m->pc < m->byte_count) {
        ...
        //printf("going around the loop, opcode %x\n", opcode);
        switch (opcode) {
        ...
        case 0x00:  // unreachable
            snprintf(exception, 1024, "%s", "unreachable");
            return false;
        case 0x01:  // nop
            continue;
        case 0x02:  // block
        ...
        case 0x0e:  // br_table
        {
            ...
            depths = (uint32_t*) acalloc(count, sizeof(uint32_t), "uint32_t");
            for(uint32_t i=0; i<count; i++) {
        ...
        case 0x11:  // call_indirect
            ...
            if (m->options.mangle_table_index) {
                ...
                if (TRACE) {
                    debug("      - entries: %p, original val: 0x%x, new val: 0x%x\n",
                        m->table.entries, val, m->table.entries - (uint32_t*)val);
                }
                val = (uint32_t*)val - m->table.entries;
            }
            if (val < 0 || val >= m->table.maximum) {
                snprintf(exception, 1024, "undefined element 0x%x", val);
                return false;
            }
            ...
            if (fidx < m->import_count) {
                ...
                if (ftype->param_count + func->local_count != m->sp - m->fp + 1) {
                    snprintf(exception, 1024, "indirect call signature mismatch");
                    return false;
                }
        ...
        case 0x28 ... 0x35:
            ...
            if (!m->options.disable_memory_bounds) {
                if (overflow) {
                    ...
                    snprintf(exception, 1024,"out of bounds memory access");
                    return false;
                }
            }
        ...
        case 0x6a ... 0x78:
            ...
            if (opcode >= 0x6d && opcode <= 0x70 && b == 0) {
                snprintf(exception, 1024, "integer divide by zero");
                return false;
            }
            switch (opcode) {
            ...
            case 0x6d: if (a == 0x80000000 && b == -1) {
                           snprintf(exception,1024, "integer overflow");
                           return false;
                       }
        ...
        case 0xa8: if (isnan(stack[m->sp].value.f32)) {
                       snprintf(exception, 1024,"invalid conversion to integer");
                       return false;
        ...
        default:
            snprintf(exception,1024, "unrecognized opcode 0x%x", opcode);
            return false;
        }
    }
}
...
Module *load_module(uint32_t len, uint8_t* file_contents, char *path, Options options) {
    uint32_t  mod_len;
    uint8_t  *bytes;
    uint8_t   vt;
    uint32_t  pos = 0, word;
    ...
    //word = read_uint32(bytes, &pos);printf("checkpoint\n");
    ASSERT(word == WA_MAGIC, "Wrong module magic 0x%x %x %x\n", word, bytes[0], WA_MAGIC);
    //word = read_uint32(bytes, &pos);
    memcpy(&word, &bytes[4], 4);
    ASSERT(word == WA_VERSION, "Wrong module version 0x%x %x\n", word, WA_VERSION);
    pos = 8;
    ...
    while (pos < mod_len) {
        ...
        case 2:
        {
            ...
            for (uint32_t gidx=0; gidx<import_count; gidx++) {
                ...
                void *val;
                char *err;
                char *sym = (char*) malloc(module_len + field_len + 5);
                ...
                do {
                    ...
                    // Also, replace '-' with '_'
                    snprintf(sym, 1024, "_%s__%s_", import_module, import_field);
                    int sidx = -1;
                    while (sym[++sidx]) {
                    ...
                    if (m->options.dlsym_trim_underscore &&
                        ...
                        snprintf(sym, 1024, "%s", import_field+1);
                        if (resolvesym(NULL, sym, &val, &err)) { break; }
                    }

                    // Try the plain symbol by itself with module name/handle
                    snprintf(sym, 1024,"%s", import_field);
                    if (resolvesym(NULL, sym, &val, &err)) { break; }
                ...
                switch (external_kind) {
                case 0x00:  // Function
                {/* just removing this and hoping it works out...
        ...
        case 5:
        {
            ...
            m->memory.bytes = (uint8_t*) acalloc(m->memory.pages*pow(2,16),
                                    sizeof(uint32_t),
                                    "Module->memory.bytes");
            //}
        }
            ...
            for(uint32_t c=0; c<element_count; c++) {
                ...
                if (m->options.mangle_table_index) {
                    ...
                    debug("   origin offset offset: 0x%x, new offset: 0x%x\n",
                          offset, (uint32_t*)offset - m->table.entries);
                    offset = (uint32_t*)offset - m->table.entries;
                }
